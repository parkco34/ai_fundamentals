#!/usr/bin/env python
"""
Learning from code I found on github.com:  https://github.com/enesozeren/decision_tree_from_scratch/blob/main/decision_tree.py
...
class DecisionTree():
    ...
    def entropy(self, class_probabilities: list) -> float:
        """
        For a given set of class probabilities
        """
    ...
    def data_entropy(self, labels: list) -> float:
        """
        Entropy based on the or class labels
        """
        return self.entropy(self.class_probabilities(labels))

    def partition_entropy(self, subsets: list) -> float:
        """
        Calculates the weighted average entropy of multiple susbets of data
        (children).
        Used to get the overall entropy after spliting the data based on a
        certain attribute.
        ------------------------------------------------------
